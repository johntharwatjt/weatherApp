...
const data = async (searchTerm, num = 0) => {
  try {
    ...
    const resLonLat = await axios.get(
...
const dateTime = async (lat, lon) => {
  try {
    const result = await axios.get(
      `https://api.ipgeolocation.io/timezone?apiKey=${clockApiKey}&lat=${lat}&long=${lon}`
    );
    ...
    if (time24[1] < 6 && time24[1] < 19) {
      sunMoon.innerHTML =
        '<img src="assets/sun.svg " alt="" width="200px" height="200px" sun-moon">';
    } else {
      sunMoon.innerHTML =
        '<img src="assets/moon.svg " alt="" width="200px" height="200px" sun-moon">';
    }
  } catch (e) {
...
const img = document.querySelector(".design");
const darkLight = document.querySelector(".dark-light-mode");
let counter = 0;
darkMode.addEventListener("click", () => {
  // document.body.style.backgroundColor = "black";
  document.body.classList.toggle("bg-black");
  countryName.classList.toggle("txt-white");
  monthdDay.classList.toggle("txt-white");
  dayClass.classList.toggle("txt-white");
  faSearch.classList.toggle("txt-white");
  counter++;
  console.log(counter);
  if (counter % 2 == 0) {
    ...
    img.src = "assets/logo-darkmode.svg";
